...
class mediator: public selector_impl
{
public:
	mediator(sql_query_string const & column_filter, sql_query_string const & select_from_expr_, sql_query_string const & mms_sql_query): 
	...
	{
		for ( auto const & flt : select_clause.filters ) if ( flt.first != col_filter ) throw MEDIATOR_MAIN_FILTER;
	}
