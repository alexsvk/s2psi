...
namespace mms
{
	...
	*/
	...
	/*!
	\brief Selects data using sql_query from a data base located in db_path and manages the allocated memory using mem_mgr.
	\param sql_query MMS SQL query.
	\param db_path The path to a data base.
	\param mem_mgr The memory manager of the returned data.
	\returns Newly allocated data.
	\pre mem_mgr should contain memory_manager::mem_info type and methods get_identifiers() which returns the managed memory IDs and manage( unique_id, memory_manager::mem_info ).
	...
	*/
	...
	select_return_val_t select_impl(sql_query_string const & sql_query, ::std::string const & db_path)
	{
		typename selector::data_deque_container data;
		try
		{
			selector selector_( sql_query );
		...
		{
			return reinterpret_cast<select_return_val_t>( BOOST_LEXICAL_CAST ); 
		}
		catch ( ::std::bad_alloc & ) // -> ::std::exception
		{
			return reinterpret_cast<select_return_val_t>( STD_PUSH_BACK_MEM_ALLOC );
		}
		int err_code = 0;
		select_return_val_t data_raw_ptr = nullptr;
		if ( ( err_code = mms::allocate( data, &data_raw_ptr ) ) != SUCCESS ) return reinterpret_cast<select_return_val_t>( err_code );
		return data_raw_ptr;	
	}
	...
	*/
	...
	insert_return_val insert_impl(sql_query_string const & sql_query, ::std::string const & db_path)
	{
		...
		catch ( insert_return_val const & code )
		{
			return code;
		}
	}
